...
function createSortTable() {
    let tableStr = "";
    tableStr += "<table>";
    tableStr += '<col style="width: 25%">';
    tableStr += '<col style="width: 50%">';
    tableStr += '<col style="width: 25%">';
    users.forEach((user) => {
        tableStr += `
            <tr>
...
app.post("/registerForm", (req, res) => {
    ...
    if (users.some((obj) => obj.login === login)) {
        res.send("Sorry, user with this login already exists");
    } else {
        users.push({
            id: id++,
            login,
        ...
        res.send(`${login}, registration successful!`);
    }
});
